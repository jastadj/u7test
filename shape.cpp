...
Shape::Shape(std::vector<uint8_t> record, std::vector<Palette> &pals)
{
    ...
    for(int i = 0; i < frame_count; i++)
    {
        ...
        // create image and offset
        m_Images.push_back(sf::Image());
        m_Offsets.push_back(sf::Vector2f(offset_x, offset_y));
        ...
        m_Images.back().create(max_x+offset_x+1, max_y+offset_y+1, sf::Color::Transparent);
        ...
        while(1)
        {
            ...
            if( uncompressed)
            {
                for(unsigned int n = 0; n < len; n++)
                {
                    ...
                    m_Images.back().setPixel(start_x+n, start_y, pals[0].colors[ record[span_offset+n] ]);
                }
                span_offset += len;
            }
            ...
            else
            {
                ...
                while(block_offset < len)
                {
                    ...
                    if(is_raw)
                    {
                        ...
                        for(int n = 0; n < run_len; n++) m_Images.back().setPixel(start_x+n+block_offset,start_y, pals[0].colors[ record[span_offset+n] ]);
                        span_offset += run_len;
                    }
                    ...
                    else
                    {
                        ...
                        for(int n = 0; n < run_len; n++) m_Images.back().setPixel(start_x+n+block_offset,start_y, pals[0].colors[ record[span_offset] ]);
                        span_offset++;
                    }
